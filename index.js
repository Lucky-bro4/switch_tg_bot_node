...
3. В течение 2-х часов курьер привезет сет вещей на указанный адрес\n
...
2. Написать на почту hello.switch@yandex.ru, ответим максимально оперативно.`
      ...
      bot.on('callback_query', msg => {
        ...
        if (text === 'Как работает Switch?' || 'Связаться с нами' || 'Заполните форму 🗒') {
          return
        } else {
          bot.forwardMessage(chatId, chatId, message_id)
          bot.onReplyToMessage(chatId, message_id)
        }
      })
      //   const {chat, message_id, text} = query.message

      //   await bot.sendMessage(chat, 'Записываю ✍️')

      //   bot.answerCallbackQuery({options: query.id})
      // })
